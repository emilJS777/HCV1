from src.services_db import firm_user_db, firm_db, user_db
...
def get_user_ids_by_firm_id(firm_id):
    ...
    user_ids = firm_user_db.get_user_ids_by_firm_id(firm_id=firm_id)
...
def bind_firm_user(firm_id, user_id):
    ...
    if not user_db.get_by_id_creator_id(user_id=user_id, creator_id=g.user_id) \
            or not firm_db.get_by_id_client_id(firm_id=firm_id, client_id=g.client_id):
    ...
    if firm_user_db.get_by_user_id_firm_id(user_id=user_id, firm_id=firm_id):
        return response(False, {'msg': 'this firm has such user'}, 409)

    #  ELSE ASSIGN USER FIRM ID
    firm_user_db.create_bind(firm_id=firm_id, user_id=user_id)
...
def unbind_firm_user(firm_id, user_id):
    ...
    if not firm_user_db.get_by_user_id_firm_id(user_id=user_id, firm_id=firm_id):
