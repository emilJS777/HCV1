...
def role_create(role_name):
    ...
    if role_db.get_by_name_creator_id(name=role_name, creator_id=g.user_id):
        return response(False, {'msg': 'role name is taken'}, 409)

    # ELSE ROLE BY THIS NAME SAVE
    role_db.create(name=role_name, creator_id=g.user_id)
...
def role_get_all():
    ...
    roles = role_db.get_all_by_creator_id(creator_id=g.user_id)
...
def role_update(role_id, role_name):
    ...
    role_db.update(role_id=role_id, creator_id=g.user_id, name=role_name)
...
def role_delete(role_id):
    ...
    role_permission_db.delete_all_by_role_id(role_id=role_id)

    # GETS ALL CONNECTIONS WITH USERS AND REMOVE
    user_role_db.delete_all_by_role_id(role_id=role_id)

    # REMOVE THIS ROLE FROM DB
    role_db.delete(role_id=role_id)
