from src.services_db import permission_db, role_permission_db, user_role_db
...
def permission_get_by_id(permission_id):
    ...
    permission = permission_db.get_by_id(permission_id)
    if not permission:
...
def permission_get_all():
    ...
    permission_ids = []
    for role_id in user_role_db.get_role_ids_by_user_id(user_id=g.user_id):
        for permission_id in role_permission_db.get_permission_ids_by_role_id(role_id=role_id):
...
def permission_update(permission_id, permission_name):
    ...
    if not permission_db.get_by_id(permission_id):
        return response(False, {'msg': 'permission by this id not found'}, 404)

    # ELSE CHANGE AND UPDATE DB
    # AND RETURN RESPONSE OK
    permission_db.update(permission_id, permission_name)
...
def permission_delete(permission_id):
    ...
    role_permission_db.delete_all_by_permission_id(permission_id)
    permission_db.delete(permission_id)
